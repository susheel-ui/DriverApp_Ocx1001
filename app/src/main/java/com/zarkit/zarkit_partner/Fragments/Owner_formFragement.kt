...
class Owner_formFragement : Fragment() {
    ...
    private fun openCamera(type: String) {
        ...
        when (type) {
            ...
            "pan" -> panUri = uri
            "driver_photo" -> driverPhotoUri = uri
        }
    ...
    fun submitOwnerDetails(onSuccess: () -> Unit) {
        ...
        when {
            ...
            driverPhotoUri == null -> {
                toast("Upload Driver Photo")
                return
            }
        }

        val jwt = LocalStorage.getToken(requireContext()) ?: return
